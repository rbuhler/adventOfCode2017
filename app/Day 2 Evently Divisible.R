  ...
  for (lineNumber in 1 : numberRows){
    ...
    for (itemPosition in 1 : (numberItems - 1)){
      ...
      for (neighborPosition in (itemPosition + 1) : numberItems){
        ...
        # print (paste0("Quociente   : ", quocient))
...
numberRows = 3
numberCols = 4
arrayValues<-c(5,9,2,8,9,4,7,3,3,8,6,5)

entry = mat <-matrix(arrayValues, 
                     nrow  = numberRows, 
                     ncol  = numberCols, 
                     byrow = TRUE)

actual   = myEventlyDivisible(entry)
expected = 9
